...
def main():
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		# train for one epoch
		acc=train(trainloader, model, criterion, optimizer, epoch)
		acc=acc/128
		acc=acc*100
		print(acc)
		accv=accu_test(testloader, model, criterion)
		print(accv)
		train_acc_list.append(acc)
		val_acc_list.append(accv)
...
def train(trainloader, model, criterion, optimizer, epoch):
	...
	for i, (input, target) in enumerate(trainloader):
		...
		# compute output
		output = model(input)
		output_1=torch.argmax(output.detach(),dim=1)
		correct_1 = (output_1==target).sum()
		#correct=correct/args.batch_size

		acc.append(correct_1)
	...
	#print('len:',len(acc))
	return torch.mean(torch.FloatTensor(acc)).numpy()




def accu_test(val_loader, model, criterion):
	model.eval()
	accv=[]
	with torch.no_grad():
	...
	return torch.mean(torch.FloatTensor(accv)).numpy()
